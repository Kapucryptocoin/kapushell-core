...
core_configure_database ()
{
    ...
    read -p "Enter the database name, or press ENTER for the default [kapu_${CORE_NETWORK}]: " inputDatabase
    ...
    if [[ ! -z "$inputDatabase" ]]; then
        ...
        sed -i -e "s/ARK_DB_DATABASE=$ARK_DB_DATABASE/ARK_DB_DATABASE=kapu_${CORE_NETWORK}/g" "$envFile"
    fi
