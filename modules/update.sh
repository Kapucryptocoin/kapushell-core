...
commander_update ()
{
    ...
    if [[ "$remote_version" == "$local_version" ]]; then
        ...
        info "You already have the latest KAPU Commander version that we support."
    else
        STATUS_COMMANDER_UPDATE="Yes"

        read -p "An update is available for KAPU Commander, do you want to install it? [Y/n] : " choice
