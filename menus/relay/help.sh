...
menu_manage_relay_help ()
{
    ...
    text_white "    stops the KAPU relay node process (shown only if the relay node is running)."
    echo

    text_yellow "    Start Relay (S)"
    text_white "    starts the KAPU relay node process (shown only if the relay node is not running)."
    echo
