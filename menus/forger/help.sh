...
menu_manage_forger_help ()
{
    ...
    text_white "    stops the KAPU forger node process (shown only if the forger node is running)."
    echo

    text_yellow "    Start Forger (S)"
    text_white "    starts the KAPU forger node process (shown only if the forger node is not running)."
    echo
